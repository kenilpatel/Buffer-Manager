    ...
    //the page is in the pool so it cannot be null.
      if(tempfd != null)
      {
        //checking the pin count of frame descriptor
          ...
          //remove page as it's pin count is 0, remove the page, updating its pin count and dirty status, the ClockPolicy and notifying replacer.
          pagemap.remove(Integer.valueOf(pageno.pid));
          tempfd.pageno.pid = INVALID_PAGEID;
          tempfd.pincnt = 0;
          tempfd.dirty = false;
          tempfd.state = ClockPolicy.AVAILABLE;
  ...
  {  
    ...
    if(tempfd != null)
    {
      //if the page is in the pool and already pinned then by using PIN_MEMCPY(true) throws an exception "Page pinned PIN_MEMCPY not allowed" 
          ...
          {
            //else the page is in the pool and has not been pinned so incrementing the pincount and setting ClockPolicy status to pinned
            tempfd.pincnt++;
            tempfd.state = ClockPolicy.PINNED;
    ...
    //if page is not present an exception is thrown as "Page not present"
      ...
      else
      {
        //unpinning the page by decrementing pincount and updating the frame descriptor and notifying replacer
          ...
          tempfd.dirty = dirty;
          if(tempfd.pincnt== 0)
          tempfd.state = ClockPolicy.REFERENCED;
